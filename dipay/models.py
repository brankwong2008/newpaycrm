...
import threading
from rbac.utils.common import compress_image_task,convert_img_jpg
from django.conf import settings
from dipay.utils.storage import ImageStorage
...
class Inwardpay(models.Model):
    ...
    def __str__(self):
        ...
        return "%s: %s %s%s (实到：%s)" % (
        create_date, self.customer, self.currency.icon, str(self.amount), str(self.got_amount))
...
class Pay2Orders(models.Model):
    ...
    rate = models.DecimalField(max_digits=9, decimal_places=4, verbose_name='转换率', default=1)
