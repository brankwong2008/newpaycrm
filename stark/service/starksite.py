...
class Option:
    def __init__(self, field,
                 ...
                 is_multi=False,
                 default=None,  # 默认选项
                 verbose_name=None,  # 筛选的字段verbose
                 control_list=None,  # 只有在list中的标签才能显示
                 extra=[],  # 额外定制字段
                 ):
        """
    ...
    class RenderData:
        ...
        def __iter__(self):
            ...
            val_list = self.query_dict.getlist(self.field)
            ...
            for item in self.data:
                ...
                tag = self.get_tag(query_dict, text, val, val_list)
        ...
        def get_tag(self, query_dict, text, val, val_list):
            ...
            return '<a href="?%s" class="%s">%s</a>' % (query_dict.urlencode(), is_active, text)
    ...
    def get_data(self, model_class, query_dict, ):
        ...
        if isinstance(field_obj, ForeignKey) or isinstance(field_obj, ManyToManyField):
            ...
            return self.RenderData(temp_model.objects.filter(**self.filter_param), self.field, verbose_name, query_dict,
                                   self)
        else:
            self.is_choice = True
...
class StarkHandler(object):
        ...
        for option in self.option_group:
            if option.is_multi:
                ...
                if query_dict.get(option.field):
                    val = query_dict.get(option.field)
                    if len(val.split("__")) == 2:
                        suffix, val = val.split("__")
                        filter_condition[option.field + "__" + suffix] = val
                    else:
                        filter_condition[option.field] = val
    ...
    def create_list(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            return JsonResponse(
                {'status': True, "data": {"pk": instance.pk, "title": str(instance), 'id_name': id_name}})
    ...
    def edit_del_display(self, obj=None, is_header=False, *args, **kwargs):
        ...
        return mark_safe(
            "<a href='%s' class='hidden-xs'><i class='fa fa-edit'></i></a> <a href='%s'><i class='fa fa-trash'></i></a>" % (
                edit_url, del_url))
